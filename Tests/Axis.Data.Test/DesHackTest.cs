...
public class DesHackTest {
  ...
  public void Decrypt() {
    ...
    _output.WriteLine("1:{0}", content);
    byte[] base64 = Convert.FromBase64String(content);
    _output.WriteLine("2:{0}", Encoding.UTF8.GetString(base64));
    // 解密器
    var des = (byte[] base64) => {
    ...
    _output.WriteLine("3:{0}", value);
    // 反轉
    string valueR = string.Join("", value.Reverse());
    _output.WriteLine("4:{0}", valueR);
    // 互換
    int[] values = valueR
      .Split(" ")
      .Select((x, y) => new { x, y })
    ...
    _output.WriteLine("5:{0}", string.Join(" ", values.Select(x => x)));
    // 轉為文字
    string text = string.Join("", values.Select(x => Char.ConvertFromUtf32(x)));
    _output.WriteLine("6:{0}", text);
  ...
  public void Encrypt() {
    ...
    _output.WriteLine("1:{0}", string.Join(" ", values.Select(x => x)));
    // 互換
    string[] valurR = values
      .Select((x, y) => new { x, y })
      .GroupBy(g => g.y / 2)
      .Select(s => s.Select(w => w.x.ToString()).Reverse()).SelectMany(i => i).ToArray();
    _output.WriteLine("2:{0}", string.Join(" ", valurR));
    // 空格並反轉
    ...
    _output.WriteLine("4:{0}", Encoding.UTF8.GetString(data));
    string content = Convert.ToBase64String(data);
    _output.WriteLine("5:{0}", content);
