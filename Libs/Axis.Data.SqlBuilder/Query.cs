...
public partial class Query : BaseQuery<Query> {
  ...
  public string QueryAlias { get; set; } = default!;
  ...
  /// <returns></returns>
  public Query ForPage(int skip, int perPage = 15) {
    return Skip(skip).Take(perPage);
  }
  ...
  private IEnumerable<KeyValuePair<string, object>> BuildKeyValuePairsFromObject(object data, bool considerKeys = false) {
    ...
    foreach (var property in props) {
      ...
      if (property.GetCustomAttributes().Any(x => x.GetType().Name == "DatabaseGeneratedAttribute")) {
        continue;
      }
