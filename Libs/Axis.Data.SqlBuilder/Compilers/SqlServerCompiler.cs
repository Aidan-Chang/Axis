...
public class SqlServerCompiler : Compiler {
  ...
  protected override string CompileColumns(SqlResult ctx) {
    ...
    if (limit > 0 && offset == 0) {
      ...
      if (compiled.IndexOf("SELECT DISTINCT") == 0) {
        return string.Concat("SELECT DISTINCT TOP (?)", compiled.AsSpan(15));
      }
      return string.Concat("SELECT TOP (?)", compiled.AsSpan(6));
    }
    return compiled;
  }
  ...
  protected override string CompileBasicDateCondition(SqlResult ctx, BasicDateCondition condition) {
    ...
    string left = part switch {
      "TIME" or "DATE" => $"CAST({column} AS {part.ToUpperInvariant()})",
      _ => $"DATEPART({part.ToUpperInvariant()}, {column})",
    };
