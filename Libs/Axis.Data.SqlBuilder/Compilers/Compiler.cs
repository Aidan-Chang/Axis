...
public partial class Compiler {
  ...
  public virtual string EngineCode { get; } = default!;
  ...
  protected virtual SqlResult CompileRaw(Query query) {
    SqlResult ctx = default!;
    ctx = query.Method switch {
      "insert" => CompileInsertQuery(query),
      "update" => CompileUpdateQuery(query),
      "delete" => CompileUpdateQuery(query),
      "aggregate" => CompileSelectQuery(
                      TransformAggregateQuery(
                        query
                          .ClearComponent("limit")
                          .ClearComponent("order")
                          .ClearComponent("group"))),
      _ => CompileSelectQuery(query),
    };
    // handle CTEs
