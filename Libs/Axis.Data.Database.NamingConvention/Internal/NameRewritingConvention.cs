...
public class NameRewritingConvention :
      ...
      foreach (var property in entityType.GetProperties()) {
        ...
        if (columnName.StartsWith(entityType.ShortName() + '_', StringComparison.Ordinal)) {
          property.Builder.HasColumnName(
            string.Concat(_namingNameRewriter.RewriteName(entityType.ShortName()), columnName.AsSpan(entityType.ShortName().Length)));
        }
  ...
  private void RewriteColumnName(IConventionPropertyBuilder propertyBuilder) {
    ...
    var baseColumnName =
      StoreObjectIdentifier.Create(property.DeclaringEntityType, StoreObjectType.Table) is { } tableIdentifier ?
